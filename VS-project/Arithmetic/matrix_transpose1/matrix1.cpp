/*鉴于第一版的一些问题现做出如下修改：
为了能使两个可视化函数合并为一个这里不再传入显示的二维矩阵而传入压缩后的三元组和行列信息。
这样就可以解决转置前后n值发生改变不能传入一个函数的问题了。
还有就是上一版中是先可视化再压缩存储于三元组因为这里要使用三元组所以先压缩进三元组再进行可视化
这一版已经有一定程度的封装，要想对其它矩阵进行转置只需修改行列数值及矩阵参数即可
*/
