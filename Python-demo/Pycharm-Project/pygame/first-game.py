...
def draw_init(node, v):  # 初始绘制蛇身，传入蛇身节点信息，及速度信息以便绘制节点圆弧，node【0】存放节点个数信息,
    # 绘制蛇头及蛇尾：
    pygame.draw.circle(screen, (255, 255, 255), (int(node[1][0]) + 1, int(node[1][1]) + 1), 5, 0)
    pygame.draw.circle(screen, (0, 0, 0), (int(node[1][0]) + 1, int(node[1][1]) + 1), 2, 0)

    pygame.draw.circle(screen, (255, 255, 255), (int(node[2][0]) + 1, int(node[2][1]) + 1), 5,
                       0)  # 失效，并不起实际作用！！！游戏开始后在没有操作前均没有绘制出蛇尾
...
def draw_initr(node, v):  # 绘制蛇身,传入更新的蛇身属性信息和速度信息，以便绘制蛇身
    for i in range(2, node[0] + 1):
        # 分别绘制头尾及及节点弧形：
        if i != 2 and i != node[0] + 1:  # 除头节点和尾节点
            pygame.draw.circle(screen, (255, 255, 255), (int(node[i][0]) + 1, int(node[i][1]) + 1), 5, 0)
        elif i == 2:  # 头节点和尾节点画半圆
            pygame.draw.circle(screen, (255, 255, 255), (int(node[1][0]) + 1, int(node[1][1]) + 1), 5, 0)
            pygame.draw.circle(screen, (0, 0, 0), (int(node[1][0]) + 1, int(node[1][1]) + 1), 3, 0)
        else:
            pygame.draw.circle(screen, (255, 255, 255), (int(node[i][0]) + 1, int(node[i][1]) + 1), 5, 0)
        # 绘制蛇身：
...
def produce_food(foodr, size, count, gatism):  # 产生随机食物的函数,传入储存食物的列表，窗口大小和产生随机食物的个数,及食物的最大大小
    for i in range(count):
        foodi = {}
        foodi['x'] = random.randint(0, size[0] - gatism[0])  # 食物坐标随机
        foodi['y'] = random.randint(0, size[1] - gatism[1])

        foodi['dx'] = random.randint(5, gatism[0])  # 食物大小随机，产生的随机数作为食物的大小并不是食物的坐标
        foodi['dy'] = random.randint(5, gatism[1])
...
def move_food(foodr):  # 绘制食物，矩形，传入食物字典
    for food in foodr:
        ...
        pygame.draw.circle(screen, (food['r'], food['g'], food['b']), (food['x'], food['y']), food['dx'], 0)
...
def judge_food(foodr, head, v):  # 传入储存食物的列表，蛇头结点的坐标，这个函数直接不对蛇节点做更改，仅判断是否有食物被吃有则删除被吃食物，并且返回True，否则返回false
    for i in range(len(foodr)):  # 为了便于删除这里用数字索引而不直接对列表内容遍历
        if foodr[i]['x'] - v / 2 <= head[0] <= foodr[i]['dx'] + foodr[i]['x'] + v / 3 and foodr[i]['y'] - v / 3 <= head[
            1] <= foodr[i]['dy'] + foodr[i]['y'] + v / 2:
            dr = foodr[i]['dx'] * foodr[i]['dy']  # 如果有食物被吃则返回食物的面积
            del foodr[i]
            return dr
    return 0
...
def written_message(self):  # 显示当前游戏信息
    fontObj0 = pygame.font.Font('simsun.ttc', 5)
    fontObj1 = pygame.font.Font('simsun.ttc', 5)
    fontObj2 = pygame.font.Font('simsun.ttc', 5)
    fontObj3 = pygame.font.Font('simsun.ttc', 5)
    fontObj4 = pygame.font.Font('simsun.ttc', 5)

    textSurfaceObj0 = fontObj0.render(u'HELLO MONCHHICHI', True, (255, 255, 255))
    textSurfaceObj1 = fontObj1.render(u'HELLO MONCHHICHI', True, (255, 255, 255))
    textSurfaceObj2 = fontObj2.render(u'HELLO MONCHHICHI', True, (255, 255, 255))
    textSurfaceObj3 = fontObj3.render(u'HELLO MONCHHICHI', True, (255, 255, 255))
    textSurfaceObj4 = fontObj4.render(u'HELLO MONCHHICHI', True, (255, 255, 255))

    textRectObj0 = textSurfaceObj0.get_rect()  # 获得要显示的对象的rect
    ...
    textRectObj0.center = (2, 2)  # 设置显示对象的坐标
    self.screen.blit(textSurfaceObj0, textRectObj0)  # 绘制字
...
while Sign:  # 开始游戏的循环
    ...
    if len(food) < foodcount and foodbool:  # 如果没有食物信息则产生随机食物
        produce_food(food, size, random.randint(2, foodcount - len(food) + 1),
                     gatism)  # 为了避免食物被全部吃完而又未产生新食物的情况将所有食物的随机最小值设为2
    ...
    #    written_message(screen)     #绘制文字信息出现问题
