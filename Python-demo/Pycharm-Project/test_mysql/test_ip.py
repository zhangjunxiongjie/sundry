...
for i in range(1, 700):
    find_result = None
    sql = "select * from daili_3 where id={}".format(i)
    cursor.execute(sql)  # 使用execute()方法执行SQL语句
    find_result = cursor.fetchone()  # 取得一条数据
    if find_result is None:
        continue
    poxy = find_result[2] + "://" + find_result[0] + ":" + str(find_result[1])
    print(poxy)
    ...
    try:
        ...
        res = requests.get(url="http://icanhazip.com/", timeout=8, proxies={find_result[2]: poxy}, headers=headers)
        proxyIP = res.text
