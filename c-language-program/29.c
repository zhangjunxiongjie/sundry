...
{
  case 'a':tc=xsw(n);break;
  case 'b':tc=dsw(n);break;
  case 'c':tc=esw(n);break;
  case 'd':tc=ssw(n);break;
  case 'e':tc=lsw(n);break;
  case 'f':tc=ybw(n);break;
  default:printf("输入错误！");   //当输入的利润值不在求解范围时，输出‘输入错误’
}
printf("得到提成为：%d\n",tc);
return 0;
}
