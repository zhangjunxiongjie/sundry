...
{
    //需要返回的4字节报告的缓冲
    //Buf[0]的 D0 就是左键，D1 就是右键，D2 就是中键（这里没有）
    //Buf[1]为 X 轴，Buf[2]为 Y 轴，Buf[3]为 滚轮
    uint8 Buf[4] = {0, 0, 0, 0};
    ...
    //如果有按键按住，并且不是 KEY7, KEY8（左, 右键） (KeyPress & (~(KEY7|KEY8)))
    //或者 KEY7, KEY8 任何一个键有变动的话，则需要返回报告 KeyUp || KeyDown
    if((KeyPress & (~(KEY7|KEY8))) || KeyUp || KeyDown)
    {
        ...
        D12WriteEndpointBuffer(USB_ENDPOINT1_IN, 4, Buf);
...
void main(void)  //主函数
{
	...
	EA = 1; //打开中断
	// 按键初始化之后 会在定时器0 中断处理时对按键进行周期扫描
	InitKeyboard(); //初始化按键 定时器 0 用于 按键扫描
	
	InitUART();  //初始化串口 定时器 1 用于 给串口提供周期脉冲
	...
	while(1)  //死循环
	{
		...
		if(ConfigValue != 0) //如果已经设置为非0的配置，则可以返回报告数据
		{
			...
			{
				// 按键扫描 在 定时计数器 中断中进行 周期执行
				KeyCanChange = 0;  //禁止按键扫描 临界区
