...
void UsbEp0SendData(void)
{
 ...
 {
  //按最大包长度发送
  D12WriteEndpointBuffer(USB_ENDPOINT0_IN,DeviceDescriptor[7],pSendData);
  //发送后剩余字节数减少最大包长
 ...
 else
 {
  ...
  {
   //不够最大包长，可以直接发送
   D12WriteEndpointBuffer(USB_ENDPOINT0_IN,SendLength,pSendData);
  ...
  else //如果要发送的数据包长度为0
  {
   ...
   {
    D12WriteEndpointBuffer(USB_ENDPOINT0_IN,0,pSendData); //发送0长度数据包
...
void UsbEp0Out(void)
{
	...
	if(D12ReadEndpointLastStatus(USB_ENDPOINT0_OUT) & 0x20) // setup 
	{
		D12ReadEndpointBuffer(USB_ENDPOINT0_OUT, 16, Buffer); //读建立过程数据
		D12AcknowledgeSetup(); //应答建立包
		D12ClearBuffer(); //清 USB 接口芯片的缓冲区
	...
	//普通数据输出
	else //if(D12ReadEndpointLastStatus(USB_ENDPOINT0_OUT)&0x20) 之 else
	{
		D12ReadEndpointBuffer(USB_ENDPOINT0_OUT,16,Buffer);
		D12ClearBuffer();
	}
}
////////////////////////End of function//////////////////////////////
...
void UsbEp0In(void)
{
	...
	D12ReadEndpointLastStatus(USB_ENDPOINT0_IN);
...
void UsbEp1In(void)
{
 ...
 D12ReadEndpointLastStatus(USB_ENDPOINT1_IN);
 //端点1输入处于空闲状态
 Ep1InIsBusy=0;
}
////////////////////////End of function//////////////////////////////
