...
//PDIUSBD12芯片连接引脚
#define D12_DATA                  P0 // 数据 读写 端
#define D12_A0                    P3_5 // 数据 命令 选择引脚
#define D12_WR                    P3_6 // 写引脚 时钟引脚
#define D12_RD                    P3_7 // 读引脚 时钟引脚
#define D12_INT                   P3_2 // 中断引脚
...
uint8 D12ReadEndpointBuffer(USB_ENDPOINT_IO_NUMBER Endp, uint8 Len, uint8 *Buf);
uint8 D12WriteEndpointBuffer(USB_ENDPOINT_IO_NUMBER Endp, uint8 Len, uint8 *Buf);

void D12ClearBuffer(void);
uint8 D12ReadEndpointLastStatus(USB_ENDPOINT_IO_NUMBER Endp);
